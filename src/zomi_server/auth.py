...
class ZoMiUser(BaseModel):
    ...
    :type password: str
    :param roles: ZoMi Permissions (WIP)
    :type roles: List[str]
...
def check_init(func):
    ...
    def wrapper(self, *args, **kwargs):
        assert isinstance(
            self, UserDB
        ), f"self is not a UserDB instance: {self=} -- {type(self)=}"
...
class UserDB:
    ...
    def set_input(self, db_path: Path) -> None:
        ...
        assert isinstance(
            db_path, Path
        ), f"{lp} db_path is not a Path: {type(db_path)=}"
    ...
    def create_user(self, username: str, password: str, disabled: bool = False) -> bool:
        ...
        if self.table.contains(tinydb.Query().username == username):
            logger.warning(
                f"{lp} User {username} already exists, use update to change password or use enable/disable"
            )
            return False
        ...
        try:
            ...
            if len(self.table.all()) > 1:
                ...
                if self.get_user(DFLT_USER_UNAME):
                    logger.info(
                        f"{lp} ANTI LOCKOUT RULE!!! There are other entries in the DB, so there is no need to "
    ...
    def get_user(self, username: str) -> Optional[ZoMiUser]:
        ...
        if user:
            ...
            try:
                ...
                logger.error(
                    f"{lp} Could not create ZoMiUser from DB entry: {e}", exc_info=True
                )
                raise e
            else:
                if user.disabled:
                    ...
                    raise HTTPException(
                        status_code=400, detail="User has been marked as disabled"
                    )
            return user
        else:
            logger.warning(f"{lp} User {username} not found")
        ...
        if not cfg.server.auth.enabled and not bypass:
            ...
            if self.init:
                ...
                if ip is None:
                    ip = "N/A"
                if not user:
                    logger.warning(f"{lp} User: {username} - Does not exist - IP: {ip}")
                    return False
                if not verify_password(password, user.password):
                    logger.warning(
                        f"{lp} User: {username} - Password is incorrect - IP: {ip}"
                    )
                    return False
            else:
...
async def verify_token(
    ...
    ret_: bool = False
    logger.info(f"{lp} Attempting to verify a request from IP: {real_ip}")
    try:
        payload = jwt.decode(
            token,
