...
from ....Models.config import DetectionResults, Result, TorchModelConfig
...
class TorchDetector:
    ...
    def __init__(self, model_config: TorchModelConfig):
        if any([torch is None, torchvision is None]):
            raise ImportError(
                f"{LP} Torch or Torchvision not installed, cannot use Torch detectors"
            )
        global g
    ...
    def warm_up(self):
        """Warm up the Torch model by running a dummy inference."""
        lp = f"{LP}{self.name}:"
        pass
    ...
    def load_model(self):
        if torch is None:
            ...
            if self.config.pretrained and self.config.pretrained.enabled is True:
                ...
                logger.warning(
                    f"{LP} pretrained was not defined, user trained models are: Not Implemented yet...."
                )
    ...
    async def detect(self, image: np.ndarray):
        ...
        if not self.ok:
            logger.warning(
                f"{LP} Something is not 'ok' (self.ok = False), skipping inference..."
            )
        else:
            ...
            try:
                ...
                logger.error(
                    f"{LP} CUDA Out of Memory Error while detecting => {oom_exc}"
                )
                oom = True
            ...
            # This needs to be called outside the try loop for memory to be freed in OOM cases
            if oom:
                if self.processor == ModelProcessor.GPU:
                    logger.debug(
                        f"{LP} OOM detected.... Attempting to clear GPU memory"
                    )
                    torch.cuda.empty_cache()
                else:
                    logger.warning(
                        f"{LP} OOM detected.... cannot clear memory on CPU, try a lower performance model"
                    )
