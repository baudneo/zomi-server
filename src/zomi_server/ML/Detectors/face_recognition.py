...
from ...Models.Enums import ModelProcessor, FaceRecognitionLibModelTypes
...
class FaceRecognitionLibDetector:
    ...
    def __init__(
        ...
        # Model init params
        self.config: Union[BaseModelConfig, FaceRecognitionLibModelConfig] = (
            model_config
        )
        self.detection_options: Optional[FaceRecognitionLibModelDetectionOptions] = (
            self.config.detection_options
        )
        self.training_options: Optional[FaceRecognitionLibModelTrainingOptions] = (
        ...
        if face_recognition is None:
            ...
            if not len(self.face_locations):
                logger.debug(
                    ...
                    f"{time.time() - detect_start_timer:.5f} s"
                )
            else:
                ...
                if not self.knn:
                    ...
                    logger.debug(
                        ...
                        f"{time.time() - comparing_timer:.5f} s"
                    )
                    if self.scaled:
                        logger.debug(
                            f"{LP} scaling bounding boxes as image was resized for detection"
                    ...
                    for pred, loc, rec in zip(
                        ...
                        labels.append(label)
                    logger.debug(
                        f"perf:{LP} recognition sequence took {time.time() - comparing_timer:.5f} s"
                    )
    ...
    def train(self, face_resize_width: Optional[int] = None):
        ...
        if not len(known_face_names):
            ...
            logger.debug(f"perf:{LP} training model took {time.time() - timer:.5f} s")
            try:
        ...
        logger.debug(f"perf:{LP} Recognition training took: {time.time() - t:.5f} s")
