...
from ....Models.config import DetectionResults, TRTModelConfig, TRTModelOptions
...
class TensorRtTorchDetector:
    class TRTModule(torch.nn.Module):
        ...
        def __init__(
            ...
            if not trt:
                logger.error(
                    f"{LP} TensorRT not installed, cannot use TensorRT detectors"
                )
                return
            super(self).__init__()
        ...
        def forward(self, *inputs) -> Union[Tuple, torch.Tensor]:
            ...
            return (
                tuple(outputs[i] for i in self.idx) if len(outputs) > 1 else outputs[0]
            )
        ...
        if not self.engine:
            ...
            for _model in get_global_config().available_models:
                if _model.id == self.id:
                    ...
                    break
            logger.error(f"cannot create detector for {self.config.name}")
            return
