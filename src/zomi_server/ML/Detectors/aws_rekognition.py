# AWS Rekognition support for ZoMi object detection
...
from ...Models.config import RekognitionModelConfig, DetectionResults, Result
...
class AWSRekognition:
    ...
    def __init__(self, model_config: RekognitionModelConfig):
        """AWS Rekognition wrapper for ZoMi ML"""
    ...
    async def detect(self, input_image: np.ndarray):
        ...
        _conf = _conf * 100
        response = self.model.detect_labels(
            Image={"Bytes": input_image.tobytes()}, MinConfidence=_conf
        )
        logger.debug(
            f"perf:{LP} took {time.time() - t} -> detection response -> {response}"
        )
