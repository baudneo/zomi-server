...
class TpuDetector:
    ...
    def warm_up(self):
        """Warm up the TPU by running a dummy inference."""
        lp = f"{LP}{self.name}:"
        runs = 2
        if self.model:
            logger.debug(f"{lp} warming up the model with {runs} iterations...")
            # create a random image to warm up TPU
    ...
    def __init__(self, model_config: TPUModelConfig):
        ...
        if pycoral is None:
            raise ImportError(
                f"{LP} pycoral is not installed, cannot use TPU detectors"
            )
    ...
    async def detect(self, input_image: np.ndarray):
        ...
        _, scale = common.set_resized_input(
            ...
            # use cv2 resize
            lambda size: cv2.resize(input_image, size),
        )
        ...
        try:
            ...
            logger.debug(
                f"perf:{LP} '{self.name}' detection took: {time.time() - t:.5f}s"
            )
            _obj_len = len(objs)
