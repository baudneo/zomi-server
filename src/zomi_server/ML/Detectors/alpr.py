...
from ...Models.Enums import ModelProcessor, ALPRAPIType
...
class PlateRecognizer(AlprBase):
    # plate rec: API response JSON={'processing_time': 84.586, 'results':
    # [{'box': {'xmin': 370, 'ymin': 171, 'xmax': 726, 'ymax': 310}, 'plate': 'cft4539'
    #   , 'region': {'code': 'ca-ab', 'score': 0.607}, 'score': 0.901, 'candidates': [
    #   {'score': 0.901, 'plate': 'cft4539'}], 'dscore': 0.76, 'vehicle': {'score': 0.244, 'type': 'Sedan', 'box':
    #   {'xmin': 49, 'ymin': 75, 'xmax': 770, 'ymax': 418}}}],
    #   'filename': '0517_HpKIJ_94bReShZAkFqUXRs-alpr.jpg', 'version': 1,
    #   'camera_id': None, 'timestamp': '2021-09-12T05:17:16.788039Z'}]
    def __init__(self, model_config: ALPRModelConfig):
...
class OpenAlprCmdLine(AlprBase):
    ...
    async def detect(self, input_image: np.ndarray):
        ...
        alpr_cmdline_exc_start = time.time()
        ...
        try:
            ...
            logger.debug(
                f"perf:{self.lp} took {time.time() - alpr_cmdline_exc_start} seconds"
            )
